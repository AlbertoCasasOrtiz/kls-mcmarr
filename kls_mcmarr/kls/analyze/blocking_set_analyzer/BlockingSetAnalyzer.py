from kls_mcmarr.kls.analyze.blocking_set_analyzer.UpperBlockAnalyzer import UpperBlockAnalyzer
from kls_mcmarr.kls.analyze.blocking_set_analyzer.InnerBlockAnalyzer import InnerBlockAnalyzer
from kls_mcmarr.kls.analyze.blocking_set_analyzer.OuterExtendedBlockAnalyzer import OuterExtendedBlockAnalyzer
...
class BlockingSetAnalyzer:
    ...
    def analyze_movement(self, modeled_movement, expected_movement, output_path, num_iter, uuid_name):
        ...
        # If all required body parts have been detected, proceed.
            ...
            if expected_movement is not None:
                if expected_movement.get_name() == "Upper Block" or expected_movement.get_name() == "Bloqueo Hacia Arriba":
                    analyzer = UpperBlockAnalyzer(modeled_movement, expected_movement.get_name())
                    ...
                    analyzer = DownwardOutwardBlockAnalyzer(modeled_movement, expected_movement.get_name())
                    analyzer.apply_rules()
                elif expected_movement.get_name() == "Rear Elbow Block" or expected_movement.get_name() == "Amartillamiento":
                    analyzer = RearElbowBlockAnalyzer(modeled_movement, expected_movement.get_name())
                    analyzer.apply_rules()
        ...
        file = open(output_path + uuid_name + ".txt", 'w')

...
class RecordWithMediapipe(Thread):
    ...
    def capture(self):
        ...
        with self.mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5, model_complexity=2) as pose:
    ...
    def store_frame_in_json(self, mediapipe_result, frame_number):
        ...
        for data_point in mediapipe_result.pose_landmarks.landmark:
            ...
            self.json_captured_data['frame ' + str(frame_number)][mediapipe_landmarks[landmark_number]]['x'] = data_point.x
            self.json_captured_data['frame ' + str(frame_number)][mediapipe_landmarks[landmark_number]]['y'] = data_point.y
            self.json_captured_data['frame ' + str(frame_number)][mediapipe_landmarks[landmark_number]]['z'] = data_point.z
            self.json_captured_data['frame ' + str(frame_number)][mediapipe_landmarks[landmark_number]][

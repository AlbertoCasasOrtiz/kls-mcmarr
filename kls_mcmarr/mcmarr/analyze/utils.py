...
def is_clockwise(x1, y1, x2, y2, c1=0, c2=0):
...
def get_points_and_check_clockwise(tested_part, reference_part, modeled_movement, pos):
...
def follows_circular_clockwise_trajectory(tested_part, reference_part, modeled_movement, pos):
    # Check if clockwise. If it is, clockwise or collinear, return true.
    clockwise = get_points_and_check_clockwise(tested_part, reference_part, modeled_movement, pos)
    return clockwise or clockwise is None


def follows_circular_counterclockwise_trajectory(tested_part, reference_part, modeled_movement, pos):
...
def store_error(errors, message, priority):
    if [message, priority] not in errors:
        errors.append([message, priority])
